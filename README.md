...
- **填過的站自動收成一行摘要**（`出生當下 · 有哭聲 · 良好`），只有「現在填這站」展開；點站名可手動展開／收合。摘要是唯讀的，不是第二份紀錄。時間線是**手風琴**：正在填的那一站釘住不會因為資料變了就被收起（9/20 住院醫師回報「點任一內容即跳下一步驟」），但一開始填下一站，上一站就回到自動規則收成一行——任何時候只有一站展開。去處未選時最後一站叫「入院」，提示回第 1 區選 NICU／NBC／BR。
...
- 表單保留手寫 chart 的區塊順序；第 5 區依「出生觀察 → 實際處置／再評估 → 產房最後狀態」填寫，輸出仍是完整英文敘述。Admission 與 Acceptance 的產房段**同一個順序**：出生觀察 → Apgar → （故事 B 的產房會診句）→ 實際處置 → 再評估（9/20 順八個情境時發現兩份 note 把 Apgar 放在段首／段尾不一致、會診句寫在急救之後與時間線矛盾，已統一）。會診時間晚於第一個處置的分鐘數時，時間線核對會提醒。
...
Admission note 結尾的 `Pedigree:` 之後接一棵純文字家庭樹，直接貼進 HIS。HIS 是記事本式的純文字、等寬環境，**對齊只能靠空格**，所以渲染器把符號放進「欄位畫布」，再依每個字元的顯示寬度補 ASCII 空格、去掉行尾空白。

- **兩種樣式**（第 6 區「樣式」，記在 `localStorage` 的 `nicu_ped_style`，預設 ASCII）：
  - **ASCII**：`[ ]` 男、`( )` 女、`< >` 性別未知，線是 `-`／`|`／`+`，近親婚 `=`，本人箭頭 `->`。全部是 1 欄寬的 ASCII，**任何等寬字型都對齊**，是預設值。
  - **符號**：`□ ○ ◇`、`─ │ ┬ ┴ ┼ ┌ ┐`、近親婚 `═`、箭頭 `→`。這些字元和 CJK 一樣在中文等寬字型（細明體等）下算 **2 欄**，渲染器就是按 2 欄排的；好看但只在全形寬度正確的字型下對齊。**貼到 HIS 後若歪掉，就換另一種樣式**——這是兩種樣式存在的唯一理由。
  - ⚠ **符號樣式要以 HIS 為準，不要看預覽**：預覽的等寬字型（`ui-monospace`／Menlo 等西文等寬）把 □○─│ 當成 1 欄畫，所以符號樹在預覽裡看起來會歪，貼進中文等寬的 HIS 反而是對的。ASCII 樣式兩邊都對齊，所以是預設值。
- **本人（proband）自動畫在最後**，性別沿用第 1 區，年齡固定 `NB`，一律加箭頭。母親年齡與 G/P/A 沿用第 1 區（`#matAge`／`#gravida`／`#para`／`#abortion`），父親年齡在第 6 區另填。
- **帶病畫實心**：父母讀第 2 區的家族史（`S.parent.thal`／`g6pd`／`thyroid`），實心之外在該側註記行接上病名（`Father 35y, G6PD deficiency`）；手足有自己的「帶病」開關。
- **同胎畫分叉**：勾了「同胎」的手足與本人合成一個子代單元，共用一條主幹再分叉；其餘手足各佔一個單元，依出生序由左至右（最年長在前）。
- **沒填的一律省略，不留佔位**：年齡、G/P/A、備註缺了就整段不寫。樹裡永遠不會出現 `__`（複製鈕的「尚有 N 空格」用 `_{2,}` 統計，樹裡有底線會被誤算）。什麼都沒填時仍畫最小樹（父母＋本人）——那是結構，不是捏造事實。
- 只進 Admission，不進 acceptance／plan／procedure。預覽用等寬字型顯示（`.note .pedigree`），複製走 `innerText`，空格會原樣帶走。
- 已知限制：符號樣式**加同胎組**時，兩人同胎組的中心落在奇數欄，接點會往左靠一欄（2 欄寬的 `┬` 必須與符號同奇偶）。同胎個案建議用 ASCII 樣式。
...
- 預覽**預設校閱模式**（Ryan 2026-09-20：醫師編輯時應顯示校閱）：填寫與手改期間看得到自動帶入、空格、手改的標記；閱讀模式手動切換給最後通讀。若在閱讀模式點進病歷手改，自動切回校閱。三段字級與兩種模式都只改顯示，不改病歷文字；更新時保留預覽位置並短暫標示變動文字，手改鎖定機制維持不變。
