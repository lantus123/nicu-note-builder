...
Admission note 結尾 `Pedigree:` 之後由 `buildPedigree()` 產生純文字樹，內容一律 escape 後包在 `<span class="pedigree">`；只進 Admission。樹只有符號一種樣式（`□ ○ ◇`／`─ │ ┬ ┴ ┼ ┌ ┐`／`→`，帶病實心；符號、線與接點一律 2 欄寬，年齡／備註的 ASCII 1 欄），沒有樣式切換。缺的資訊整段省略，不補佔位，樹內不得出現 `__`（複製鈕以 `_{2,}` 統計未填空格）。
